...
{
	//マップの黒い画像。
	//m_spriteRender.Init("Assets/sprite/MAP.dds", 512, 720);
	m_spriteRender.SetPosition(MAP_CENTER_POSITION);

	//マップ中心の赤い画像。
	//m_playerSprite.Init("Assets/sprite/PlayerPosition.dds", 50, 50);
	m_playerSprite.SetPosition(MAP_CENTER_POSITION);

	//エネミーを表す、青い画像。
	//m_enemySprite.Init("Assets/sprite/EnemyPosition.dds", 35, 35);
...
void Map::Update()
{
	...
	Vector3 enemyPosition = m_player->GetPosition();
